...
Vi kan beregne sandsynligheden for at en celle i afstand $r$ udfra.

\begin{equation}
	P(r) = \eta \lim \limits_{\rho \to 0} \bigint_{r-\rho}^{r+\rho} \frac{1}{\sqrt{2 \pi \big(\frac{\alpha}{6}\big)^2}}e^{- \frac{(x - z_t)^2}{2 (\frac{\alpha}{6})^2}}\, \mathrm{d}x
\end{equation}
\\
Normaliseringskonstanten $\eta$ kan vi bestemme da vi ved at vi vil have sandsynligheden fra $l_{occ}$ i centrum for normalfordelingen.
For at genskabe sandsynligheden udfra $l_{occ}$ benytter vi os af \cref{logodds:bel}.
...
l_{r} = \begin{cases} 
	...
	log\Big(\frac{P(r)}{1-P(r)}\Big) &\text{hvis } z_t-\frac{\alpha}{2} \leq r \leq z_t+\frac{\alpha}{2}\\ 
	l_{free} &\text{ellers}	
\end{cases}
\end{equation}
