...
\thilemann{Jeg har dummet mig og markeret namespaces med fed istedet for lstinline}
...
Som det første beskrives \lstinline!CommonLib.dll! i \cref{arkitektur:commonlib}, da systemets lag alle holder referencer hertil.
...
\caption{Lagene i systemarkitekturen med deres ydre associationer. Associationerne til højre fører alle til \lstinline!CommonLib.dll!, som ses på \cref{arkitektur:klassediagram:2}.}
...
Derfor holder det også en reference til namespacet \lstinline!NKH.MindSqualls! for at kunne tilgå den del af MindSqualls (\cref{mindsqualls}) som indeholder kommunikationen til NXT-enheden.
...
Som navnet på namespacet antyder, så er formålet med \lstinline!CommonLib.NXTPostMan! at sende beskeder frem og tilbage mellem PC og robotten; hvilket gør det ansvarlig for al kommunikation imellem de to, som beskrives detaljeret i \cref{kommunikation}.
...
Den benytter så henholdsvis \lstinline[style=csharp]|LocationControl| og \lstinline[style=csharp]|MappingControl| til at reagere på beskeder den finder, og anmoder dermed \lstinline!Services! namespacet om at udføre den pågældende handling.
...
Kald hertil stammer fra \lstinline!SystemInterface!, der anmoder om en action, som dette namespace så reagerer på ved at benytte \lstinline!Services! namespacet.
...
Dette er det nederste lag i arkitekturen og benyttes af \lstinline!Control! namespacet når en handling ønskes udført.
...
For at få robotten til at køre en rute på kortet benyttes \lstinline!RouteService! namespacet, der består af en grafisk komponent, der gør det muligt at klikke en rute ind på et occupancy grid samt en \lstinline[style=csharp]|SchedulingService|, der returnerer en kø af de punkter, som ruten består af.

\paragraph{Services.KinectServices}
Benyttes af \lstinline!Control.DisplayControl! for at vise en videostrøm fra farvekameraet i Kinecten.
Konverterer den returnerede videostrøm til en strøm af bitmaps, som kan vises grafisk i \lstinline!SystemInterface!.

\paragraph{Services.TrackingServices}
Denne service er ansvarlig for at holde styr på robottens lokation samt positur.
Robotten kan således gennem \lstinline!Control! benytte dette namespace, når den ønsker en opdatering af sin positur.
...
Alt data opdateres fra \lstinline!Control! når robotten har taget en ny måling.
