...
\anders{Log odds er for at undgå at beyes regel skal kører fast på sansynlighederne 1 eller 0}s
...
Opdaterings-algoritmen tager \textit{log odds} for en \textit{posterior belief} $l_{t-1}$ for en celle $m_i$, samt en ny sensor-måling $z_t$, hvor ud fra der returneres en \textit{log odds} for en ny \textit{belief} $l_t$ for $m_i$.
