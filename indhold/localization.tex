...
Her er $v$ den synsvinkel som kameraet har i den givne dimension; for Kinect'en er det 57\dg i bredden og 43\dg i højden.
$d$ er afstand mellem kamera og den overflade der skal bruges.
...
\coordinate (B) at (2,6);
\coordinate (C) at (4,0);
\coordinate (mAC) at (2,0);
\coordinate (A2) at (1,3);
\coordinate (C2) at (3,3);

\draw [dashed] (mAC) -- (B) node [midway,left,yshift=-1cm] {$d$};
\draw (A) -- (B);
\draw (B) -- (C);
\draw (A) -- (C) node [midway,below] {$x_2$};
\draw (A2) -- (C2) node [midway,below,xshift=0.3cm] {$x_1$};
